...
const loadProducts = () => {
  ...
  showProducts(data);//products show function
};
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) { //every single products loop
    const image = product.image; //image 
    const div = document.createElement("div"); //create div
    div.classList.add("product"); //new add 
    div.innerHTML = `<div class="single-product">
      ...
      `;
    document.getElementById("all-products").appendChild(div);  //all products div
  }
};
let count = 0;
const addToCart = (id, price) => {
  //console.log(id)
  count = count + 1;
  updatePrice("price", price)  //update price function

  updateTaxAndCharge(); //update tax charge function
  document.getElementById("total-Products").innerText = count;
  updateTotal() //total result show function call
};
const getInputValue = (id) => {
  const element = document.getElementById(id).innerText
  const converted = (parseInt(element * 100) / 100)
  return converted;
};
